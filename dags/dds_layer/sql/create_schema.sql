...
CREATE TABLE IF NOT EXISTS "{{ params.schema_name }}".scills_group (
	id INT PRIMARY KEY,
	scill_group_name VARCHAR(50) NOT NULL,
    UNIQUE (scill_group_name)
);
...
CREATE TABLE IF NOT EXISTS "{{ params.schema_name }}".skills (
    ...
    CONSTRAINT fk_skills_scills_group
        FOREIGN KEY(group_id)
            REFERENCES "{{ params.schema_name }}".scills_group(id),
